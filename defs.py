...
def announce(message, to_group_1=False, level_1=1, to_group_2=False, level_2=1):
    ...
    # Compose messages for groups 1 and 2 (usually Telegram and LaMetric)
    message_group_1 = f"{message} ({config.symbol})"
    message_group_2 = f"{message}"
    ...
    if to_group_1:
        announce_helper(config.notify_1_enabled, config.notify_1_level, level_1, "primary", message_group_1)

    # Output to Apprise Group 2 (Usually LaMetric)
    if to_group_2:
        announce_helper(config.notify_2_enabled, config.notify_2_level, level_2, "secondary", message_group_2)
